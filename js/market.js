...
function openSellModal(itemId) {
    ...
    // Ищем предмет в инвентаре напрямую
    const item = state.inventory.find(i =>
        (i.id === itemId) || (i.item_id === itemId)
    );

    if (!item) {
        // Если не нашли в инвентаре, пробуем найти в ITEMS
        const itemFromCatalog = getItemById(itemId);
        if (!itemFromCatalog) {
            showToast('Предмет не найден', 'error');
            return;
        }
        // Используем данные из каталога
        pendingSellItemId = itemFromCatalog.id;
    } else {
        // Используем данные из инвентаря
        pendingSellItemId = item.id || item.item_id;
    }
    ...
    const displayItem = item || getItemById(itemId);
    nameElement.textContent = `${displayItem.icon} ${displayItem.name}`;
    priceInput.value = displayItem.price;
