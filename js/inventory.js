...
function renderInventory() {
    ...
    // Группировка по item_id (а не по id)
    const groups = {};
    state.inventory.forEach(item => {
        const key = item.id || item.item_id;
        if (!groups[key]) {
            groups[key] = { ...item, qty: 0 };
        }
        groups[key].qty++;
    });
        ...
        div.innerHTML = `
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <button onclick="sellItem(${item.id || item.item_id})" class="bg-red-500/20 text-red-300 px-3 py-2 rounded-lg text-sm font-bold hover:bg-red-500/30 transition">
                        💰 Продать
                    </button>
                    <button onclick="openSellModal(${item.id || item.item_id})" class="bg-yellow-500/20 text-yellow-300 px-3 py-2 rounded-lg text-sm font-bold hover:bg-yellow-500/30 transition">
                        📊 Выставить
                    </button>
                </div>
            </div>
...
function sellItem(itemId) {
    ...
    // В Telegram Mini App confirm() может не работать
